...
@section('content')
    ...
    <div class="glass rounded-2xl overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-dark-700/50">
                        <th class="text-left text-xs font-medium text-dark-400 uppercase tracking-wider px-6 py-4">Client
                        </th>
                        <th class="text-left text-xs font-medium text-dark-400 uppercase tracking-wider px-6 py-4">Bisnis
                        </th>
                        <th class="text-left text-xs font-medium text-dark-400 uppercase tracking-wider px-6 py-4">Industry
                        </th>
                        <th class="text-left text-xs font-medium text-dark-400 uppercase tracking-wider px-6 py-4">Projects
                        </th>
                        <th class="text-left text-xs font-medium text-dark-400 uppercase tracking-wider px-6 py-4">Lifetime
                            Value</th>
                ...
                <tbody class="divide-y divide-dark-700/30">
                    @forelse($clients as $client)
                        <tr class="table-row">
                            <td class="px-6 py-4">
                                ...
                                <p class="text-xs text-dark-400">{{ $client->email }}</p>
                            </td>
                            <td class="px-6 py-4 text-sm text-dark-300">{{ $client->business_name }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-dark-300">{{ $client->projects_count ?? 0 }} project</td>
                            <td class="px-6 py-4 text-sm font-medium text-emerald-400">Rp
                                {{ number_format($client->total_lifetime_value, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="badge px-2.5 py-1 rounded-lg text-xs font-medium {{ $client->client_status == 'active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-dark-600 text-dark-400' }}">{{ ucfirst($client->client_status) }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('clients.show', $client) }}"
                                        class="p-1.5 text-dark-400 hover:text-blue-400 transition-colors"><svg class="w-4 h-4"
                                    ...
                                    <a href="{{ route('clients.edit', $client) }}"
                                        class="p-1.5 text-dark-400 hover:text-amber-400 transition-colors"><svg class="w-4 h-4"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-dark-400 py-8">Belum ada client</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($clients->hasPages())
        <div class="px-6 py-4 border-t border-dark-700/50">{{ $clients->withQueryString()->links() }}</div> @endif
    </div>
@endsection