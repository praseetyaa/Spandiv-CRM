        ...
        <div class="glass rounded-2xl p-8 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-dark-800/50 rounded-xl p-4">
                    <p class="text-xs text-dark-400 mb-1">Email</p>
                    <p class="text-sm text-white">{{ $client->email }}</p>
                </div>
                <div class="bg-dark-800/50 rounded-xl p-4">
                    <p class="text-xs text-dark-400 mb-1">Telepon</p>
                    <p class="text-sm text-white">{{ $client->phone }}</p>
                </div>
                <div class="bg-dark-800/50 rounded-xl p-4">
                    <p class="text-xs text-dark-400 mb-1">Instagram</p>
                    <p class="text-sm text-white">{{ $client->instagram ?? '-' }}</p>
                </div>
                <div class="bg-dark-800/50 rounded-xl p-4">
                    <p class="text-xs text-dark-400 mb-1">Lifetime Value</p>
                ...
                @forelse($client->projects as $project)
                    <div class="bg-dark-800/50 rounded-xl p-4 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-dark-400">{{ $project->service->name }} · Rp
                ...
                @forelse($client->invoices as $invoice)
                    <div class="bg-dark-800/50 rounded-xl p-4 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-dark-400">{{ $invoice->issue_date->format('d M Y') }} · Rp
