...
@section('content')
    {{-- Welcome Banner --}}
    <div class="welcome-banner rounded-2xl p-6 mb-6 text-white relative overflow-hidden animate-fade-up">
        <div class="absolute inset-0 opacity-10">
            <svg class="w-full h-full" viewBox="0 0 400 200" fill="none">
                <circle cx="350" cy="20" r="100" fill="white" />
        ...
        <div class="relative z-10 flex items-center justify-between">
            <div>
                @php
                    ...
                    $quotes = [
                        'Setiap lead adalah peluang baru! 🚀',
                        'Konsistensi adalah kunci sukses! 💪',
                        'Hari yang baik untuk closing deal! 🎯',
                        'Terus semangat, target pasti tercapai! ⭐',
                        'Great things never come from comfort zones! 🔥',
                    ];
                @endphp
                <h2 class="text-2xl font-bold mb-1">{{ $emoji }} {{ $greeting }}, {{ auth()->user()->name }}!</h2>
                <p class="text-white/80 text-sm">{{ $quotes[array_rand($quotes)] }}</p>
            </div>
            <div class="hidden md:flex items-center gap-3 text-right">
                <div>
                    <p class="text-white/70 text-xs">{{ now()->translatedFormat('l') }}</p>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                {{-- Total Leads --}}
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter"
                        data-target="{{ $stats['total_leads_this_month'] }}">0</p>
                    <p class="text-xs text-gray-500 dark:text-dark-400 mt-1">Total Lead</p>
                </div>

                {{-- Conversion Rate --}}
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-2">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter"
                        data-target="{{ $stats['conversion_rate'] }}" data-suffix="%">0</p>
                    <p class="text-xs text-gray-500 dark:text-dark-400 mt-1">Conversion Rate</p>
                </div>

                {{-- Active Projects --}}
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-3">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter"
                        data-target="{{ $stats['active_projects'] }}">0</p>
                    <p class="text-xs text-gray-500 dark:text-dark-400 mt-1">Project Berjalan</p>
                </div>

                {{-- Monthly Revenue --}}
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-4">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter-currency"
                        data-target="{{ $stats['monthly_revenue'] }}">Rp 0</p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-2">
                    <div class="flex items-center justify-between mb-3">
                        <div class="w-10 h-10 bg-pink-500/10 rounded-xl flex items-center justify-center">
                            <svg class="w-5 h-5 text-pink-500 dark:text-pink-400" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter"
                        data-target="{{ $stats['active_subscriptions'] }}">0</p>
                ...
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-3">
                    ...
                    <p class="text-2xl font-bold text-{{ $stats['overdue_invoices'] > 0 ? 'red-400' : 'gray-900 dark:text-white' }} counter"
                        data-target="{{ $stats['overdue_invoices'] }}">0</p>
                ...
                <div class="stat-card glass rounded-2xl p-5 card-hover card-glow animate-fade-up stagger-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white counter"
                        data-target="{{ $stats['total_clients'] }}">0</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="glass rounded-2xl p-6 card-hover animate-fade-up stagger-5">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">🏆 Top 5 Client</h3>
                    <div class="space-y-3">
                        @forelse($topClients as $index => $client)
                            <div
                                class="flex items-center gap-3 p-3 rounded-xl bg-gray-100 dark:bg-dark-800/50 table-row-animated">
                                <div
                                    class="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-lg shadow-blue-500/20">
                                    {{ $index + 1 }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ $client['name'] }}
                                    </p>
                                    <p class="text-xs text-gray-500 dark:text-dark-400">{{ $client['business_name'] ?? '' }}</p>
                                </div>
                                <p class="text-sm font-semibold text-emerald-500 dark:text-emerald-400">Rp
                        ...
                        @empty
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500 dark:text-dark-400">Belum ada data client</p>
                            </div>
                        @endforelse
                    </div>
                </div>

                <div class="glass rounded-2xl p-6 card-hover animate-fade-up stagger-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">📊 Performa Sumber Lead</h3>
                    <div class="space-y-3">
                        @forelse($leadSources as $source)
                            <div class="p-3 rounded-xl bg-gray-100 dark:bg-dark-800/50 table-row-animated">
                                <div class="flex items-center justify-between mb-2">
                                    <span
                                        class="text-sm font-medium text-gray-900 dark:text-white capitalize">{{ $source['source'] }}</span>
                                    <span
                                        class="text-xs text-gray-500 dark:text-dark-400">{{ $source['won'] }}/{{ $source['total'] }}
                                        won</span>
                                </div>
                                <div class="w-full bg-gray-200 dark:bg-dark-700 rounded-full h-2 overflow-hidden">
                                    <div class="bg-gradient-to-r from-blue-500 to-purple-500 h-2 rounded-full transition-all duration-1000 ease-out"
                        ...
                        @empty
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500 dark:text-dark-400">Belum ada data lead</p>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        {{-- RIGHT COLUMN: Charts --}}
        <div class="space-y-6">
            <div class="glass rounded-2xl p-6 card-hover animate-fade-up stagger-3">
...
@push('scripts')
    <script>
        ...
        function animateCurrencyCounter(el) {
            const target = parseFloat(el.dataset.target) || 0;
            const duration = 1500;
            const start = performance.now();
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (entry.target.classList.contains('counter-currency')) animateCurrencyCounter(entry.target);
                    observer.unobserve(entry.target);
                }
            });
        }, { threshold: 0.5 });

        document.querySelectorAll('.counter, .counter-currency').forEach(el => observer.observe(el));
        ...
        window.launchConfetti = function () {
            ...
            for (let i = 0; i < 60; i++) {
                ...
                if (shapes[Math.floor(Math.random() * 2)] === 'circle') piece.style.borderRadius = '50%';
        ...
        new Chart(document.getElementById('revenueChart'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    pointBorderColor: isDark ? '#0f172a' : '#ffffff', pointBorderWidth: 2,
                }]
            },
            options: {
        ...
        new Chart(document.getElementById('categoryChart'), {
            ...
            data: {
                ...
                datasets: [{ data: categoryData.map(d => parseFloat(d.total_revenue)), backgroundColor: categoryData.map(d => categoryColors[d.category] || '#64748b'), borderWidth: 0, hoverOffset: 8 }]
            },
            options: {
