        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-dark-700/50">
                    ...
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Tipe</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Total</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Terbayar</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Status</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Due</th>
            ...
            <tbody class="divide-y divide-dark-700/30">
                @forelse($invoices as $inv)
                    ...
                    <tr class="table-row">
                        <td class="px-6 py-4">
                            ...
                            <p class="text-xs text-dark-400">{{ $inv->issue_date->format('d M Y') }}</p>
                        </td>
                        <td class="px-6 py-4 text-sm text-dark-300">{{ $inv->client->name }}</td>
                        <td class="px-6 py-4"><span
                                class="text-xs {{ $inv->subscription_id ? 'text-cyan-400' : 'text-dark-400' }}">{{ $inv->subscription_id ? '🔁 Recurring' : '📋 One Time' }}</span>
                        </td>
                        <td class="px-6 py-4 text-sm text-white font-medium">Rp
                ...
                @empty <tr>
                    <td colspan="8" class="text-center text-dark-400 py-8">Belum ada invoice</td>
                </tr>@endforelse
            </tbody>
        </table>
        @if($invoices->hasPages())
        <div class="px-6 py-4 border-t border-dark-700/50">{{ $invoices->withQueryString()->links() }}</div>@endif
    </div>
@endsection