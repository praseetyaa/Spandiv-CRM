        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-200 dark:border-dark-700/50">
                    <th class="text-left text-xs font-medium text-gray-500 dark:text-dark-400 uppercase px-6 py-4">Project</th>
                    ...
                    <th class="text-left text-xs font-medium text-gray-500 dark:text-dark-400 uppercase px-6 py-4">Progress</th>
                    <th class="text-left text-xs font-medium text-gray-500 dark:text-dark-400 uppercase px-6 py-4">Deadline</th>
                    <th class="text-left text-xs font-medium text-gray-500 dark:text-dark-400 uppercase px-6 py-4">Harga</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-dark-700/30">
                    ...
                    <tr class="table-row">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <div class="w-20 bg-gray-200 dark:bg-dark-700 rounded-full h-1.5">
                                    <div class="bg-blue-500 h-1.5 rounded-full"
                                        style="width:{{ $project->progress_percentage }}%"></div>
                                </div><span class="text-xs text-gray-500 dark:text-dark-400">{{ $project->progress_percentage }}%</span>
                            </div>
                        </td>
                        <td
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-dark-300">Rp {{ number_format($project->price, 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <a href="{{ route('projects.edit', $project) }}"
                                    class="p-1.5 text-gray-400 dark:text-dark-400 hover:text-amber-400"><svg class="w-4 h-4" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty <tr>
                    <td colspan="7" class="text-center text-gray-500 dark:text-dark-400 py-8">Belum ada project</td>
                </tr>@endforelse
            </tbody>
        </table>
        @if($projects->hasPages())
        <div class="px-6 py-4 border-t border-gray-200 dark:border-dark-700/50">{{ $projects->withQueryString()->links() }}</div>@endif
    </div>
@endsection