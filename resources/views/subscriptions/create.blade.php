        ...
        <div class="glass rounded-2xl p-8">
            <form method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div><label class="block text-sm font-medium text-gray-600 dark:text-dark-300 mb-2">Harga/Bulan (Rp)
                            <span class="text-red-400">*</span></label><input type="number" name="price"
                            value="{{ old('price', $subscription->price ?? '') }}" required
                            class="w-full px-4 py-2.5 bg-gray-100 dark:bg-dark-800/50 border border-gray-300 dark:border-dark-600/50 rounded-xl text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500/50 focus:outline-none">
                    </div>
                    <div><label class="block text-sm font-medium text-gray-600 dark:text-dark-300 mb-2">Billing
                            Cycle</label><select name="billing_cycle"
                            class="w-full px-4 py-2.5 bg-gray-100 dark:bg-dark-800/50 border border-gray-300 dark:border-dark-600/50 rounded-xl text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500/50 focus:outline-none">
                    ...
                    <div><label class="block text-sm font-medium text-gray-600 dark:text-dark-300 mb-2">Tanggal
                            Akhir</label><input type="date" name="end_date"
                    ...
                    <div><label class="block text-sm font-medium text-gray-600 dark:text-dark-300 mb-2">Status <span
                            ...
                            <option value="{{ $v }}" {{ old('status', $subscription->status ?? 'active') == $v ? 'selected' : '' }}>{{ $l }}</option>@endforeach
                        </select></div>
                </div>
