        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-dark-700/50">
                    ...
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Layanan</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Harga/Bulan</th>
                    <th class="text-left text-xs font-medium text-dark-400 uppercase px-6 py-4">Mulai</th>
            ...
            <tbody class="divide-y divide-dark-700/30">
                @forelse($subscriptions as $sub)
                    ...
                    <tr class="table-row">
                        <td class="px-6 py-4 text-sm font-medium text-white">{{ $sub->client->name }}</td>
                        <td class="px-6 py-4 text-sm text-dark-300">{{ $sub->service->name }}</td>
                        <td class="px-6 py-4 text-sm text-emerald-400 font-medium">Rp
                            {{ number_format($sub->price, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 text-sm text-dark-300">{{ $sub->start_date->format('d M Y') }}</td>
                ...
                @empty <tr>
                    <td colspan="6" class="text-center text-dark-400 py-8">Belum ada subscription</td>
                </tr>@endforelse
            </tbody>
        </table>
        @if($subscriptions->hasPages())
        <div class="px-6 py-4 border-t border-dark-700/50">{{ $subscriptions->withQueryString()->links() }}</div>@endif
    </div>
@endsection